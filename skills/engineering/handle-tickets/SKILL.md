...
2. Normalize the handoff into findings owned by this conductor:
   ...
   - For a changed PR state, run the confirmed-merge follow-up when merged, return a targeted state blocker when closed without merge, or verify head and checkpoint continuity before resuming a reopened PR.
