---
name: handle-tickets
description: "Ticket orchestrator for delivering existing implementation-ready tracker tickets through ticket conductors, fresh code review, a local Codex review/fix loop, PR publication, and Codex PR validation."
...
- PR readiness follows **PR Review Activation** below.
...
Each ticket PR has one recorded PR worker task ID from first hosted entry through final scope fit. A `codex-pr-review` terminal handoff completes one invocation without clearing that ownership. Resume the recorded task when it is available; otherwise follow the saved-task recovery and replacement rule in **Command Chain**, atomically recording exactly one replacement only after the prior task and descendants are terminal.

- **Local state:** the PR is draft. Local branch-changing and head-bound phases may run; the PR worker performs no watcher or hosted-feedback side effect.
- **Hosted state:** the PR is ready. The recorded PR worker is the sole hosted-feedback owner. It packets each observed feedback item immediately; after the conductor obtains the Scope Guard decision, the PR worker applies the required reaction and reply before any repair or phase transition. Before every watcher run or side effect, require the recorded task ID, `isDraft: false`, and matching local, remote, and PR Heads.
- **Local -> Hosted:** the conductor verifies the Head and resumes or starts the recorded PR worker while the PR is draft. The worker re-verifies the Head, captures the freshness boundary, marks the PR ready, processes current unreacted feedback, and starts the watcher.
- **Hosted -> Local:** the PR worker finishes current feedback dispositions, verifies the Head, marks the PR draft, confirms the unchanged Head and `isDraft: true`, and returns that receipt. No local or branch-changing worker starts before the receipt.
- **Ready recovery:** when local work resumes on an already-ready PR without a recorded PR worker, spawn and record exactly one PR worker on the unchanged Head, let it process current hosted feedback, then complete **Hosted -> Local** before local work.
- Final scope fit runs in Hosted state. Keep the PR worker task ID through scope fit; before recording any scope-fit PASS, resume that worker for a final hosted-status check and bounded ledger closure on the unchanged Head.
...
Local review input freezes at dispatch. The worker reviews the pinned fixed-point-to-Head diff directly; GitHub feedback cannot expand its packet, delay completion, or renew Local Codex. Require **Local state** before dispatch.
...
The conductor confirms local `HEAD` matches the remote full head SHA, then performs **Local -> Hosted** with the hosted-review input. Carry `expectedHeadRefOid` and `statusFreshAfter` exactly as `codex-pr-review` defines them.

For this caller, the PR worker returns a merge conflict to the conductor with `integration refresh` as the next transition. Complete **Hosted -> Local** before the conductor spawns a fresh implementation/fix worker for reconciliation and classifies the refresh as mechanical or substantive. An ordered PR follows the Merge Lane. For any other PR, a mechanical refresh returns to **Local Codex Review/Fix** for a current-Head checkpoint, while a substantive refresh records the integrated base as the fixed point and returns to **Code Review**. The conductor then resumes the PR worker at the resulting hosted-review entry point.
Before any integration refresh, scope correction, or other transition that can change the Head or renew a local review, complete **Hosted -> Local**. Keep Hosted state after successful validation while final scope fit runs.
...
For `Outcome: clear`, prepare `PASS`. For findings, follow **Review Decision Flow** using `## Scope fit` as the durable record; a Scope Guard decision of `clear` also prepares `PASS`, while every other decision records `FAIL` and its next transition. Before recording either PASS, complete the final hosted-status check and bounded ledger closure required by **PR Review Activation** and require the PR worker to return the unchanged reviewed Head. A closure repair invalidates the scope-fit packet and returns through the required local and hosted phases before a fresh scope-fit review. Then record `## Scope fit`, `PASS`, and `Head: <full SHA>`.
...
2. Complete **Hosted -> Local** on the unchanged Head. If hosted feedback changes the Head, invalidate the scope-fit packet and decision, complete the required hosted validation, and run a fresh scope-fit review before considering any correction.
3. The conductor spawns a fresh implementation/fix worker with the latest durable Scope Guard decision and correction classification to apply the scope correction while preserving required behavior.
4. Have that worker rerun relevant checks, commit, and push, then record the full correction head SHA.
5. Return to **Local Codex Review/Fix** for a fresh local review.
6. Run hosted validation through **Ready PR and Run Codex PR Review**.
7. Return to **Check Final Scope Fit**.
...
- The recorded code-review or PR worker task ID while its phase or review ownership remains resumable
