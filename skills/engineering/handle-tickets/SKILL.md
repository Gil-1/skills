...
Local review input freezes at dispatch. The worker reviews the pinned fixed-point-to-Head diff directly. GitHub Codex threads remain hosted feedback: their content may be evidence, but thread presence is not a local finding, and the local phase never reacts, replies, or resolves them. Record each thread URL and reviewed Head for the PR worker; feedback published after dispatch cannot expand the packet, delay completion, or renew Local Codex. A defect independently proven by the local worker follows **Review Decision Flow**, while its GitHub thread remains queued for hosted review.
...
Pass the PR worker every queued hosted-feedback URL with its original reviewed Head and any durable Scope Guard decision that independently covered the same invariant. Preserve those feedback batches in its round history for `codex-pr-review` **Repeated Patterns**; the PR worker alone applies reactions, replies, and resolutions.
