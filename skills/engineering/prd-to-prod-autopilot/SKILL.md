...
3. Create issues. Load `to-issues` with the PRD and any referenced-work inventory. If invoked after `to-prd`, do not stop at PRD publication; continue to draft the child issues. In autopilot mode, pause before issue publication so the user can review the drafted child issues; after user approval, publish them before continuing. Done when user-approved published child issues, dependencies, labels, blocked slices, and issue URLs are captured in the work queue.
