...
- The parent autopilot owns orchestration: sequencing, concurrency decisions, sub-agent handoffs, assigned worktree/branch integration, launching fresh `codex-pr-review` sub-agents for created PRs, post-merge issue reconciliation when merges happen externally, and final gates. It should not implement or review/fix issue work itself when a suitable sub-agent can do that work.
...
9. Return merge and cleanup decision: report the PRs or single delivery branch ready for human merge, timed out, or blocked, then ask the user to confirm when merges are done and whether to run post-merge cleanup.
