...
- `to-tickets` owns vertical ticket breakdown, ticket publication, and marking the slices it creates as `ready-for-agent`.
- `triage` owns raw incoming issues, external PRs, pre-existing untriaged work, tracker state transitions, and durable agent briefs. Do not re-triage tickets freshly created by `to-tickets`; only use `triage` when the PRD references existing tracker items, labels are missing or conflicting, or the work arrived outside the `to-tickets` path.
...
- Ticket-breakdown approval is a hard human gate. Present the drafted child tickets and stop until the user explicitly approves publication. Do not publish child tickets, create implementation worktrees, load `implement` for delivery work, or spawn code-changing workers before that approval.
...
3. Create tickets. Load `to-tickets` with the PRD and any referenced-work inventory. If invoked after `to-spec`, do not stop at spec publication; continue to draft the child tickets. In autopilot mode, pause before ticket publication so the user can review the drafted child tickets; after user approval, publish them before continuing. Done when user-approved published child tickets, dependencies, labels, blocked slices, and ticket URLs are captured in the work queue.
4. Gate the work queue. Verify each ticket is independently reviewable or explicitly dependent in the declared topology. Route slices with split pressure back through `to-tickets` for splitting or blocking before implementation continues. Done when every implementable ticket maps to exactly one review unit and every blocked ticket has evidence.
5. Schedule workers. First confirm the ticket-breakdown approval gate was satisfied. Assign independent `ready-for-agent` items to supervised worker sub-agents inside their assigned worktrees, and require each worker to load `implement` for the assigned issue. Launch sub-agents in parallel whenever supported. Serialize issues that have dependencies, touch likely same risky files, public contracts, migrations, data models, or shared tests. Done when every unblocked implementable issue is running, queued behind an explicit dependency, or blocked.
...
- Do not grant implementation workers push or PR-creation permission. Only a post-review publisher may perform initial push/open for the PR, and only after `review-fix` and the pre-publish repo gate are complete. PR-review fixers may push scoped fixes to an existing PR after publication.
...
- For `split_followup_issues`, route the split through `to-tickets` or existing tracker work, update the original issue and PR with the created follow-up ticket URLs, and schedule unblocked slices through the normal workflow when they are part of the current PRD scope.
