...
3. Create issues. Load `to-issues` with the PRD and any referenced-work inventory. In autopilot mode, treat `to-issues` breakdown approval as an evidence gate: approve, split, or block from the PRD and repo evidence; ask the user only for true product-scope decisions. Done when published child issues, dependencies, labels, blocked slices, and issue URLs are captured in the work queue.
