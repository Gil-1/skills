...
- Pause before publishing child issues until the user approves the issues so they can be published.
...
3. Create issues. Load `to-issues` with the PRD and any referenced-work inventory. In autopilot mode, draft the child issues and pause before publication; ask the user to approve before continuing.
