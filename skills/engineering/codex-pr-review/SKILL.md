...
- Keep PR review/fix work in a dedicated git worktree. Reuse a caller-supplied worktree after verifying PR branch and current head; otherwise create or verify one before edits/checks.
...
- Send the fixer packet: PR URL/number, branches, worktree, current `headRefOid`, the current review-cycle freshness boundary when known, watcher-fresh `feedbackItems` and `activeCodexThreads`, parsed priorities, watcher snapshot counts, P0/P1 safety-pass results when needed, Internal Review Discipline scope baseline, related prior Codex comments for repeated patterns, prior `code-review` or `review-fix` outcomes, checks, and push policy. Include enough spec context to classify findings; do not ask for a fresh full `code-review` unless a finding specifically requires it.
...
- If no fixer sub-agent can be spawned, handle active comments in the parent using Internal Review Discipline and Internal Severity Handling, run checks, and report the fallback.

Fixer sub-agent:

- Complete the supplied work directly without delegating. Refresh PR state, read the full body for every supplied item/thread, apply Internal Review Discipline and Internal Severity Handling, fix valid in-scope blockers, run checks, commit, and push.
...
Default to `internal`: follow the complete review/fix loop in this skill.
...
This mode defines transport and ownership only. The caller owns finding policy, code changes, checks, and post-change review transitions.

Internal Review Discipline and Internal Severity Handling do not classify caller-owned handoffs. In caller-owned mode, gather fresh evidence and apply only the dispositions supplied by caller receipts.
...
- In caller-owned mode, return every unresolved closure finding with current-head evidence and wait for its receipt before reacting, replying, or resolving.
- In internal mode, if a thread's finding no longer applies to the current head, record the stale or already-fixed disposition, apply the validity reaction, reply when required, and resolve it without reopening the finding.
- In internal mode, if the defect still exists on the current head, process it through Internal Review Discipline and Internal Severity Handling; any fixer commit changes the expected head and restarts hosted validation before another closure pass.
...
- In internal mode, if the coherent fix is outside the PR contract, crosses owner boundaries, requires a product decision, or needs a new public/protocol/storage contract, stop as `blocked` with the smallest targeted question and evidence. Do not rename that outcome as redesign or split.
...
- After every watcher run, process state events before any status-specific branch. Return `pr_head_changed`, `pr_state_changed`, or `merge_state_changed` raw in caller-owned mode. In internal mode, restart validation from a changed head, stop with the observed closed/merged PR state, and branch on observed mergeability: only `DIRTY` or `CONFLICTING` enters merge-conflict handling; other states remain mergeability evidence for the owning loop.
...
3. In caller-owned mode, return a merge conflict as a feedback handoff. Otherwise resolve merge conflicts before waiting for Codex, commit, push, and restart the loop.
...
5. If the watcher reports successful validation, re-inspect the PR with `gh pr view --json headRefOid,state,mergeStateStatus,statusCheckRollup` and route any head, PR-state, or merge-state event before accepting `headRefOid` as watcher `current.headRefOid`; return raw events in caller-owned mode and use the internal event transitions otherwise. Run **Review Ledger Closure** for that head. If closure changes the head, restart hosted validation; otherwise repeat the same head/state/merge inspection. Only when it remains stable may the worker run `git fetch --all --prune --tags`, inspect all three fields once more through the same event branches, report the validation mode, and avoid another review request for cleaner wording or a second opinion.
6. If Codex is reviewing, run the watcher and re-inspect the PR. If watcher output includes current-head `feedbackItems` or `activeCodexThreads`, return them as a feedback handoff in caller-owned mode; otherwise handle them through Internal Review Discipline and Internal Severity Handling. If the watcher times out while current status is still reviewing, stop and report Codex as stuck or timed out; do not start another watcher run for the same head without new input.
7. If no PR-body status exists, run the watcher with `--timeout 300` as the silent-start check and re-inspect the PR. Return any current-head feedback as a feedback handoff in caller-owned mode; otherwise handle it internally before deciding whether to request Codex.
...
9. For fresh feedback in internal mode, prefer one fixer sub-agent. Use multiple fixers only for isolated worktrees or clearly non-overlapping fixes with an explicit push order. The orchestrator delegates fixes; the parent fixes only when no fixer can be spawned.
10. After caller disposition receipts, update round history and run Repeated Patterns before continuing. After the internal fixer pushes fixes or reports no code change was needed, classify the round against the scope baseline, update round history, and run Repeated Patterns before another narrow fixer pass or review request. If an internal root-cause fix is pushed, restart from the current PR head so Codex can validate it.
11. Stop only when Codex validation succeeds, a caller-owned feedback handoff is ready, times out, blocks on GitHub, or a real blocker remains after root-cause investigation.
