...
- Send the fixer packet: PR URL/number, branches, worktree, current `headRefOid`, watcher-fresh `feedbackItems` and `activeCodexThreads`, parsed priorities, watcher snapshot counts, P0/P1 safety-pass results when needed, Review Discipline scope baseline, related prior Codex comments for repeated patterns, prior `code-review` or `review-fix` outcomes, relevant local Codex reviewed HEAD, findings, dispositions, fixes, and checks when supplied by the caller, current checks, and push policy. Include enough spec context to classify findings; use prior local evidence without rerunning or duplicating the local preflight workflow, and do not ask for a fresh full `code-review` unless a finding specifically requires it.
