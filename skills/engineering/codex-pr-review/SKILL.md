...
- If the defect still exists on the current head, return its evidence unclassified as a feedback handoff in caller-owned mode. In internal mode, process it through Review Discipline and Severity Handling; any fixer commit changes the expected head and restarts hosted validation before another closure pass.
