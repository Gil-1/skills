...
5. If the watcher reports successful validation, re-inspect the PR with `gh pr view --json headRefOid,state,isDraft,statusCheckRollup` and require its `headRefOid` to equal watcher `current.headRefOid` and `isDraft` to be `false`; if the head differs, restart from the new head with its review-cycle boundary, and if the PR is draft return to the caller for reactivation. Run **Review Ledger Closure** for that head. If closure changes the head, restart hosted validation; otherwise re-inspect `headRefOid,isDraft`, require `headRefOid` still to equal the expected head and `isDraft` to remain `false`, then run `git fetch --all --prune --tags`, report the validation mode, and do not request another review for cleaner wording or a second opinion.
