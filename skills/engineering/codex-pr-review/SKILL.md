...
4. When the caller explicitly identifies a newer local checkpoint on an unchanged expected head, run one checkpoint-refresh cycle: add one PR comment exactly `@codex review`, capture the current UTC timestamp immediately after the request as the new review-cycle freshness boundary, then run the normal watcher loop with that expected head and boundary. Consume this authorization with that request; do not enter this branch for ordinary fixer pushes or normal resumptions.
