...
4. If the watcher reports `codex_approved` or `codex_review_complete`, re-inspect the PR with `gh pr view --json headRefOid,state,statusCheckRollup`, run `git fetch --all --prune --tags`, report the terminal mode, and do not request another review for cleaner wording or a second opinion.
...
10. Stop only when the watcher reports `codex_approved` or `codex_review_complete`, times out, blocks on GitHub, or a real blocker remains after root-cause investigation.
...
- Do not ask for or infer a human override of the hosted gate. A completed dispositioned review is a defined terminal outcome; every other non-passing outcome remains a blocker.

## Terminal Reporting

- For every terminal outcome, include PR URL, whether Codex validated it, terminal mode (`codex_approved` or `codex_review_complete` when successful), `merge_ready`, observed PR head SHA, current PR state, whether local git data was fetched, commits pushed, whether a P0/P1 safety pass ran, whether a repeated-pattern investigation ran, and handled Codex comments by priority and disposition: fixed/accepted, rejected, stale/already-fixed, follow-up, blocked, or deferred, with validity reactions and no-fix rationales.
- For any terminal outcome other than `codex_approved` or `codex_review_complete`, include a continuation packet with `merge_ready: false`, PR URL, branch, base, observed PR head SHA, linked ticket or PRD URLs, current PR state, checks and mergeability, Codex disposition, clustered file/theme/comment evidence, P0/P1 dispositions, root-cause investigation notes when relevant, commits pushed, and next action plus owner.
