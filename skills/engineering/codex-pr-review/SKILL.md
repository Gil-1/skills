...
4. If the watcher reports `codex_approved` or `codex_review_complete`, confirm `gh pr view --json headRefOid,state,statusCheckRollup` and require `headRefOid` to equal watcher `current.headRefOid`; if it differs, restart from the new PR head. Then run `git fetch --all --prune --tags`, report the terminal mode, and do not request another review for cleaner wording or a second opinion.
...
10. Stop only when the current head reaches `codex_approved` or `codex_review_complete`, times out, blocks on GitHub, or a real blocker remains after root-cause investigation.
