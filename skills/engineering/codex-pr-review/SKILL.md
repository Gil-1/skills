...
- The watcher polls with `gh` and exits when Codex PR-body status, Codex feedback, or PR state changes. If it exits with `timeout`, inspect the PR once more before reporting that Codex may be stuck.
