...
4. If the watcher reports successful validation, re-inspect the PR with `gh pr view --json headRefOid,state,statusCheckRollup`, run `git fetch --all --prune --tags`, report the validation mode, and do not request another review for cleaner wording or a second opinion.
...
10. Stop only when Codex validation succeeds, times out, blocks on GitHub, or a real blocker remains after root-cause investigation.
