...
5. If the watcher reports successful validation, re-inspect the PR with `gh pr view --json headRefOid,state,statusCheckRollup` and require its `headRefOid` to equal watcher `current.headRefOid`; if it differs, return a head-changed handoff in caller-owned mode, otherwise restart from the new head with its review-cycle boundary. Run **Review Ledger Closure** for that head. If closure changes the head, restart hosted validation; otherwise re-inspect `headRefOid`. If that head differs from the expected head, return a head-changed handoff in caller-owned mode, otherwise restart internal validation. Only when it remains equal may the worker run `git fetch --all --prune --tags`, re-inspect the PR head once more through the same caller-handoff/internal-restart branch, report the validation mode, and avoid another review request for cleaner wording or a second opinion.
