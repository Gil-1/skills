...
- In the final response, include the PR URL, whether Codex validated it, `merge_ready`, the reviewed head SHA, current PR state, whether local git data was fetched, any commits pushed, whether a P0/P1 safety pass ran, and a summary of handled Codex comments with priority, validity reactions, and no-fix rationales. Include the continuation packet for every non-validated terminal outcome.
