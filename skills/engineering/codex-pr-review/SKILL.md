...
- After every watcher run, process state events before any status-specific branch. Return `pr_head_changed`, `pr_state_changed`, or `merge_state_changed` raw in caller-owned mode. In internal mode, restart validation from a changed head, stop with the observed closed/merged PR state, or re-enter merge-conflict handling for changed mergeability.
