...
- `prd-to-prod-autopilot` owns issue creation, existing tracker normalization when needed, implementation, review/fix, final repo validation, implementer-owned PR publishing, and Codex PR review.
...
5. Hand off delivery: load `prd-to-prod-autopilot` with the PRD, architecture decision packets, and any outer delivery constraints. Let it own issue creation, existing tracker normalization when needed, worker execution, review/fix, repo gate, implementer-owned PR publishing, and Codex PR review.
